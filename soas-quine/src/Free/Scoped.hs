-- copied from https://github.com/rzk-lang/rzk/blob/7d42ef6267a42fcd4577228ef56f4d3749d5d2be/rzk/src/Free/Scoped.hs
...
data Sum f g scope term
  = InL (f scope term)
  | InR (g scope term)
  deriving (Eq, Show, Functor, Foldable, Traversable, GHC.Generic)
deriveBifunctor ''Sum
...
traverseFS
  :: (Bitraversable term, Monad f)
  => (forall s t. term s t -> f (term' s t))
  -> FS term a -> f (FS term' a)
traverseFS phi = \case
  Pure x -> return (Pure x)
  Free t -> Free <$> (bitraverse (traverseFS phi) (traverseFS phi) t >>= phi)
